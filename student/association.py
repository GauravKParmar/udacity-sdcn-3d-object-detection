...
class Association:
    ...
    def associate(self, track_list, meas_list, KF):
        ...
        association_matrix = np.ones((len(track_list), len(meas_list))) * np.inf
        for tid, track in enumerate(track_list):
            for mid, meas in enumerate(meas_list):
                ...
                if self.gating(MHD, meas.sensor):
                    association_matrix[tid, mid] = MHD
    ...
    def get_closest_track_and_meas(self):
        ...
        track_idx, meas_idx = np.unravel_index(
            np.argmin(self.association_matrix), self.association_matrix.shape
        )
        if self.association_matrix.shape and np.isfinite(
            self.association_matrix[track_idx, meas_idx]
        ):
