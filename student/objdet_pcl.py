...
def show_pcl(pcl):
    ...
    window = visual.create_window(window_name='PCL_Window', width=1920, height=1080)
    ...
    def callback(visual):
        visual.close()
    visual.register_key_callback(262, callback)
    visual.run()
...
def show_range_image(frame, lidar_name):
    ...
    lidar_data = [object for object in frame.lasers if object.name == lidar_name][0]
    ...
    range = (range - range.min()) * 255 / (np.amax(range) - np.amin(range))
    
    # step 5 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
    int_min = np.quantile(intensity, 0.01)
    int_max = np.quantile(intensity, 0.99)
    intensity[intensity < int_min] = int_min
    intensity[intensity > int_max] = int_max
    intensity = (intensity - int_min) * 255 / (int_max - int_min)
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 0] = np.int_(np.floor(lidar_pcl_cpy[:, 0] / discrete_bev))

    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_pcl_cpy[:, 1] = np.int_(np.floor(lidar_pcl_cpy[:, 1] / discrete_bev) + (configs.bev_width + 1) / 2)
